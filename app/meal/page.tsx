  ...
  if (planStatus === "loading" || !today) {
    return (
      <div className="mx-auto flex min-h-screen max-w-2xl flex-col items-center justify-center gap-3 px-4 text-zinc-400">
        <div className="h-10 w-10 animate-spin rounded-full border-2 border-zinc-600 border-t-sky-400" />
  ...
  return (
    <div className="mx-auto min-h-screen max-w-2xl px-4 pb-16 pt-8">
      ...
      <div className="mt-2 flex flex-wrap items-center gap-2">
        <h1 className="text-3xl font-bold text-zinc-50">{meal.label}</h1>
        {meal.isVegetarian ? (
      ...
      <section className="mt-8 space-y-4">
        <h2 className="text-lg font-semibold text-zinc-100">All meals</h2>
        {meal.meals.map((m) => (
          <article
            key={`${m.time}-${m.name}`}
              ...
              <div className="min-w-0 flex-1">
                ...
                <dl className="mt-4 grid grid-cols-2 gap-2 text-sm sm:grid-cols-4">
                  <div>
                    ...
                    <dd className="font-semibold text-zinc-200">{m.calories}</dd>
                  </div>
                  <div>
                    <dt className="text-xs uppercase text-zinc-500">Protein</dt>
                    <dd className="font-semibold text-sky-300">{m.protein}g</dd>
                  </div>
                  <div>
                    <dt className="text-xs uppercase text-zinc-500">Carbs</dt>
                    <dd className="font-semibold text-amber-300">{m.carbs}g</dd>
                  </div>
                  <div>
                    <dt className="text-xs uppercase text-zinc-500">Fat</dt>
                    <dd className="font-semibold text-rose-300">{m.fat}g</dd>
                  </div>
                </dl>
              </div>
            </div>
